...
public class Solution {
    public int maximumWealth(int[][] accounts) {
        int[] sumsOfMoney = new int[accounts.length];
        for (int i = 0; i < accounts.length; i++) {
            for (int j = 0; j < accounts[i].length; j++)
                sumsOfMoney[i] += accounts[i][j];
        }

        int maximumInArray = sumsOfMoney[0];
        for (int k = 1; k < sumsOfMoney.length; k++) {
            if (sumsOfMoney[k] > maximumInArray)
                maximumInArray = sumsOfMoney[k];
        }
